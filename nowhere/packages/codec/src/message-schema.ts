import type { MessageData, Tag } from './types.js';
import {
	escapeStoreName,
	escapeDescription,
	escapeImage,
...
export function validateMessage(data: MessageData): void {
	if (data.pubkey && data.pubkey.length !== 43) {
		throw new ValidationError('Pubkey must be 43 characters (base64url)', 'INVALID_PUBKEY');
	}
	if (!data.name || data.name.trim() === '') {
		throw new ValidationError('Author name cannot be empty', 'EMPTY_NAME');
	}
}

// ─── Serialize ───────────────────────────────────────────────
// Format: pubkey,authorName"markdownBody:avatar<tags
// Same as store header, NO `;` separator, no items section.
...
export function deserializeMessage(str: string): MessageData {
	// Split on first unescaped `,` → pubkey, rest
	const commaPos = findNextUnescaped(str, ',', 0);
	if (commaPos === -1) {
		throw new SchemaError('Message must have at least pubkey and author name');
	}
