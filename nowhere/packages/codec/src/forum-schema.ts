import type { ForumData, Tag } from './types.js';
import {
	escapeStoreName,
	escapeDescription,
	escapeImage,
...
export function validateForum(data: ForumData): void {
	if (!data.pubkey || data.pubkey.length !== 43) {
		throw new ValidationError('Pubkey must be 43 characters (base64url)', 'INVALID_PUBKEY');
	}
	if (!data.name || data.name.trim() === '') {
		throw new ValidationError('Forum name cannot be empty', 'EMPTY_NAME');
	}
}

// ─── Serialize ───────────────────────────────────────────────
// Format: pubkey,forumName"description:image<tags

export function serializeForum(data: ForumData): string {
	validateForum(data);
...
export function deserializeForum(str: string): ForumData {
	// Split on first unescaped `,` → pubkey, rest
	const commaPos = findNextUnescaped(str, ',', 0);
	if (commaPos === -1) {
		throw new SchemaError('Forum must have at least pubkey and name');
	}
