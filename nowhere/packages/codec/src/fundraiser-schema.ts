import type { FundraiserData, Tag } from './types.js';
import {
	escapeStoreName,
	escapeDescription,
	escapeImage,
...
export function validateFundraiser(data: FundraiserData): void {
	if (data.pubkey && data.pubkey.length !== 43) {
		throw new ValidationError('Pubkey must be 43 characters (base64url)', 'INVALID_PUBKEY');
	}
	if (!data.name || data.name.trim() === '') {
		throw new ValidationError('Campaign name cannot be empty', 'EMPTY_NAME');
	}
}

// ─── Serialize ───────────────────────────────────────────────
// Format: pubkey,campaignName"description:image<tags
// Same as message schema format.

export function serializeFundraiser(data: FundraiserData): string {
	validateFundraiser(data);
...
export function deserializeFundraiser(str: string): FundraiserData {
	// Split on first unescaped `,` → pubkey, rest
	const commaPos = findNextUnescaped(str, ',', 0);
	if (commaPos === -1) {
		throw new SchemaError('Fundraiser must have at least pubkey and campaign name');
	}
