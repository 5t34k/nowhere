<script lang="ts">
	...
	import { messageBuilderState, messageEncodedResult, toMessageData } from '$lib/stores/message-builder-state';
	import { decode, serializeMessage } from '@nowhere/codec';
	...
	import MessageAuthorPanel from '$lib/components/message/MessageAuthorPanel.svelte';
	import MessageContentPanel from '$lib/components/message/MessageContentPanel.svelte';
	...
	// Invalidate encryption when message data changes
	$effect(() => {
		const currentFragment = $messageEncodedResult?.fragment;
	...
	$effect(() => {
		$messageEncodedResult?.fragment;
		publishUrl = '';
	});
	const activeUrlLength = $derived(
		publishUrl
			? publishUrl.length
			: $messageEncodedResult
				? (RENDERER_ORIGIN + '/s#' + $messageEncodedResult.fragment).length
				: lastValidLength
	);
	...
	$effect(() => {
		if ($messageEncodedResult) {
			lastValidLength = (RENDERER_ORIGIN + '/s#' + $messageEncodedResult.fragment).length;
		}
	});
	...
	function handleReset() {
		if (confirm('Reset all message data? This cannot be undone.')) {
			messageBuilderState.reset();
			lastValidLength = 0;
			encryptEnabled = false;
			encryptedFragment = '';
			encryptedSourceFragment = '';
	...
	function handleImport(fragment: string) {
		try {
			...
			if (data.siteType !== 'message') {
				console.error('Import failed: not a message');
				return;
			}
			messageBuilderState.importFromData(data as MessageData);
			showImport = false;
			editorCollapsed = false;
			activePanel = 'author';
		} catch (e) {
...
<svelte:head>
	<title>Create Message · Nowhere</title>
</svelte:head>
...
<BuilderLayout {editorCollapsed} onToggleCollapse={handleToggleCollapse} {mobileShowPreview} scrollKey={activePanel}>
	...
	{#snippet editor()}
		{#if activePanel === 'author'}
			<MessageAuthorPanel
				pubkey={$messageBuilderState.pubkey}
				name={$messageBuilderState.name}
				image={$messageBuilderState.image}
				onUpdate={(field, value) => messageBuilderState.updateField(field as any, value as any)}
			/>
		{:else if activePanel === 'content'}
			<MessageContentPanel
				description={$messageBuilderState.description}
				tags={$messageBuilderState.tags}
				onUpdate={(field, value) => messageBuilderState.updateField(field as any, value as any)}
				onTagUpdate={(tags) => messageBuilderState.updateField('tags', tags)}
			/>
		{:else if activePanel === 'tips'}
			<MessageTipsPanel
				tags={$messageBuilderState.tags}
				onUpdate={(tags) => messageBuilderState.updateField('tags', tags)}
			/>
		...
		{:else if activePanel === 'verification'}
			<VerificationPanel
				storeData={toMessageData($messageBuilderState)}
				labels={{ entityLabel: 'Message', ownerLabel: 'Author' }}
				onTagUpdate={(key, value) => {
					messageBuilderState.update(s => {
		...
		{:else if activePanel === 'publish'}
			<PublishPanel
				...
				notReadyItems={messageNotReadyItems}
				expectedPubkey={$messageBuilderState.pubkey}
			siteType="message"
			/>
		{/if}
	{/snippet}
	...
	{#snippet preview()}
		<LivePreview result={$messageEncodedResult} openUrl={encryptedPreviewUrl} />
	{/snippet}
	...
	{#snippet budget()}
		...
		>
			<CompressionView serializedText={(() => { try { const d = toMessageData($messageBuilderState); return d ? serializeMessage(d) : null; } catch { return null; } })()} encodedResult={$messageEncodedResult} label="message" />
		</CharacterBudget>
	{/snippet}
</BuilderLayout>

{#if showImport}
	<ImportDialog label="message" onImport={handleImport} onClose={() => (showImport = false)} />
{/if}
	...
	<div class="encrypt-alert">
		<div class="encrypt-alert-content">
			...
			<div>
				...
				<p>Message data changed.<br/>Go to Share Link to re-encrypt.</p>
			</div>
